...
import type { Integration, IntegrationType, CreateIntegrationData } from '@/lib/types/integration';
...
function IntegrationsPageContent() {
  ...
  const [selectedIntegrationForLogs, setSelectedIntegrationForLogs] = useState<Integration | null>(
    null
  );
    ...
    try {
      ...
      if (editingIntegration) {
        ...
        addToast({
          ...
          message: t('toast.updatedMessage', { name: data.name }),
        });
      } else {
        ...
        addToast({
          ...
          message: t('toast.createdMessage', { name: data.name }),
        });
      }
      ...
      addToast({
        type: 'error',
        title: editingIntegration ? t('toast.updateFailed') : t('toast.createFailed'),
  ...
  const handleDelete = async (integration: Integration) => {
    if (!confirm(t('deleteConfirm', { name: integration.name }))) {
      return;
    }
    ...
    try {
      ...
      addToast({
        ...
        message: t('toast.deletedMessage', { name: integration.name }),
      });
      await fetchData();
    } catch (error: any) {
      addToast({
  ...
  const handleTest = async (integration: Integration) => {
    ...
    try {
      ...
      if (result.success) {
        addToast({
          type: 'success',
          title: t('toast.connectionSuccess'),
        ...
        addToast({
          ...
          message: result.error || t('toast.unknownError'),
        });
      }
    } catch (error: any) {
      addToast({
        type: 'error',
        title: t('toast.connectionTestFailed'),
        message: error.message,
      });
    } finally {
      setTestingId(null);
  ...
  const executeSyncDirection = useCallback(
    async (direction: 'push' | 'pull' | 'both') => {
      ...
      try {
        ...
        const parts: string[] = [];
        if (result.pushed > 0) parts.push(t('toast.pushed', { count: result.pushed }));
        if (result.pulled > 0) parts.push(t('toast.pulled', { count: result.pulled }));
        if (result.skipped > 0) parts.push(t('toast.skipped', { count: result.skipped }));
        if (parts.length === 0) parts.push(t('toast.nothingToSync'));
        addToast({
          type: 'success',
          title: t('toast.syncQueued', { direction }),
          message: parts.join(', '),
        });
      } catch (error: any) {
        addToast({
  ...
  const getMostRecentSync = () => {
    ...
    if (dates.length === 0) return t('stats.never');
    const mostRecent = new Date(Math.max(...dates));
    const now = new Date();
    const seconds = Math.floor((now.getTime() - mostRecent.getTime()) / 1000);
    if (seconds < 60) return t('stats.justNow');
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {!isLoading && integrations.length === 0 && !error && (
          <EmptyState
            ...
            description={t('noIntegrationsDescription')}
        ...
        {integrations.length > 0 && (
          <>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-6">
              ...
              <div className="flex items-center gap-1 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-1.5">
                {[
                  ...
                  ...types.map(tp => ({ value: tp.type, label: tp.name })),
                ].map(opt => (
                  <button
                    key={opt.value}
              ...
              <div className="flex items-center gap-1 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-1.5">
                {[
                  ...
                  { value: 'true', label: t('filters.enabled') },
                  { value: 'false', label: t('filters.disabled') },
                ].map(opt => (
                  <button
                    key={opt.value}
            ...
            {filteredIntegrations.length === 0 ? (
              <EmptyState
                icon="🔍"
                title={t('noMatchFilter')}
                description={t('noMatchFilterDescription')}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-6 max-w-md w-full border border-gray-200 dark:border-gray-700">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">
                {t('sync.title', { name: syncTarget.name })}
              </h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                {t('sync.chooseDirection')}
              </p>
              <div className="space-y-3">
                ...
                >
                  ...
                  <div>
                    <p className="font-medium text-gray-900 dark:text-white">{t('sync.push')}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      {t('sync.pushDescription', { name: syncTarget.name })}
                    </p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    <p className="font-medium text-gray-900 dark:text-white">{t('sync.pull')}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      {t('sync.pullDescription', { name: syncTarget.name })}
                    </p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    <p className="font-medium text-gray-900 dark:text-white">
                      {t('sync.bidirectional')}
                    </p>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      {t('sync.bidirectionalDescription')}
                    </p>
                  </div>
                </button>
              </div>
