...
function MetricCard({ title, value, subtitle, accentColor, isLoading }: MetricCardProps) {
  return (
    <div
      className="bg-white dark:bg-gray-900 rounded-lg shadow p-5 border-l-4"
...
export function AgentTaskMetrics({ stats, isLoading }: AgentTaskMetricsProps) {
  const t = useTranslations('agent.metrics');
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
      ...
      <MetricCard
        ...
        value={stats?.avgResolutionTime ? `${stats.avgResolutionTime}m` : t('notAvailable')}
        subtitle={t('avgResolutionUnit')}
        accentColor="#8B5CF6"
        isLoading={isLoading}
      />
      <MetricCard
