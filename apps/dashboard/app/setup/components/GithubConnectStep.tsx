...
export default function GithubConnectStep({ onComplete, onSkip }: GithubConnectStepProps) {
  const t = useTranslations('setupGithub');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md">
        <p className="text-sm text-gray-700 dark:text-gray-300 mb-4">{t('connectNoteOrLater')}</p>
        <p className="text-xs text-gray-500 dark:text-gray-400">{t('authNote')}</p>
      </div>
      ...
      <p className="text-xs text-gray-500 dark:text-gray-400 text-center">{t('laterNote')}</p>
    </div>
  );
}
